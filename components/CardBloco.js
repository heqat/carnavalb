export default function CardBloco({ nome, dia, horario, categoria, localizacao, mapa }) {
    return (
        <div className="card border border-2 m-1 bloco">
            <div className="container-categoria bloco">
                <span className="categoria bloco">{categoria}</span>
            </div>
            <div className="card-body bloco">
                <div>
                    <span className="dia bloco">{dia}</span>
                    <h5 className="card-title bloco">{nome}</h5>
                </div>
                <div>
                    <h6 className="card-subtitle bloco"><i className='text-white bx bx-time'></i> {horario}</h6>
                    <h6 className="card-subtitle bloco"><i className='text-white bx bx-map-pin'></i> <a href={mapa} target="_blank"> {localizacao}</a></h6>
                </div>
            </div>
        </div>


    )
}